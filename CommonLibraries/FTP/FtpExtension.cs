...
namespace FTP
{
    ...
    public partial class Ftp
    {
        ...
        private string _downloadFtpDirPath;
        private long _downloadTotalFileSize;
        ...
        private void DownloadDirectory(string downloadPath, string localPath, DownloadDirectoryChangedAction action)
        {
            ...
            this._downloadFtpDirPath = downloadPath;
            this._downloadTotalFileSize = 0;
            ...
            if (rootDir != null)
            {
                ...
                foreach (var dir in this._downloadList)
                {
                    ...
                    foreach (var file in dir.Files)
                    {
                        this._downloadTotalFileSize += file.Size;
                    }
                }
        ...
        {
            var subDir = currentDir.DirPath.Replace(this._downloadFtpDirPath, "");
            string dirLocalPath = string.Format("{0}/{1}", this._downloadLocalDirPath, subDir);
        ...
        {
            var subDir = file.FullPath.Replace(this._downloadFtpDirPath, "");
            string downloadLocalPath = string.Format("{0}/{1}", this._downloadLocalDirPath, subDir);
            var args = new FtpDownloadDirectoryChangedEventArgs(this._downloadTotalCount, ++this._downloadCount, file.Name, downloadLocalPath, file.FullPath, this._downloadTotalFileSize, file.Size);
