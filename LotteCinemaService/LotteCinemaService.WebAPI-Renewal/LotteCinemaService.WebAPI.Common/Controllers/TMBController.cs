...
namespace LotteCinemaService.WebAPI.Common.Controllers
{
    public class TMBController : ApiController
    {
        ...
        {
            var result = this._cmManager.GetSpecialTypeInfo();
        ...
        //{
        //    var result = this._cmManager.GetScheduleList("1016", "202", ContentsType.SpecialAdver);

        //    if (result.Count > 0)
        //    {
        //        foreach (var item in result)
        //        {
        //            if (string.IsNullOrEmpty(item.IsFull) || item.IsFull == "0")
        //            {
        //                return 0;
        //            }
        //            else
        ...
        [HttpPost]
        [POST("TMB/SetSpecialType?{status}")]
        public bool SetSpeacialType(bool status)
        {
            return this._tmbManager.UpdateSpecialStatus(status);
        }
