...
namespace LotteCinemaService.WebAPI.TBA.Controllers
{
    public class TBAController : ApiController
    {
        ...
        public List<TransparentAdInfo> GetTransparentAdInfoList(string cinemaCode)
        {
            ...
            {
                var fileName = string.Format("{0}.{1}", item.FileName, item.FileType);
                var configFile = ConfigHelper.GetFilePath(item.ID.ToString(), fileName, ItemID.TBA, ContentsType.TransparentAd);
                ...
                TransparentAdInfo adverInfo = new TransparentAdInfo
                {
                    ...
                    FtpFilePath = configFile.FtpFilePath,
                    LocalFilePath = configFile.LocalFilePath
                };
